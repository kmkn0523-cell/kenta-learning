# ronin_reply_engine.py
# @RoninWords Threads 自動リプライ・エンジン（GitHub Actions専用・対話なし・全自動）
# keyword_search で見つけた英語のニッチ投稿（規律・ストイック・目的など）にテンプレ返信を自動で送る。
# ★skin_reply_engine.py の英語版。roninは英語アカウントなので「英語判定」でターゲットを絞る。
...
def contains_english(text):
    """文字列が英語（ターゲット言語）かどうかをざっくり判定する。
    ASCIIのアルファベットが15文字以上あれば英語の文章とみなす。
    （roninは英語アカウントなので、日本語など非英語の投稿は対象外にする）"""
...
def is_repliable(post, my_username, state, config):
    ...
    if not author:  # 著者名が取れない投稿はスキップ（連投・記録の事故防止）
        return False
    if author.lower() == (my_username or "").lower():  # 自分の投稿には返信しない（大小無視で確実に）
        return False
    if post.get("id") in state["replied_post_ids"]:  # 既に返信済み
        return False
    if state["author_counts"].get(author, 0) >= config["max_per_author_per_day"]:  # 同一著者の上限
        return False
    text = post.get("text") or ""
    if len(text) < config["min_post_length"]:  # 短すぎる投稿は中身が薄い
        return False
    if not contains_english(text):  # 英語以外（ターゲット外）
        return False
...
CONFIG_PATH = BASE_DIR / "ronin_reply_engine_config.json"  # キーワード・上限設定（読むだけ）
STATE_PATH = BASE_DIR / "ronin_reply_history.json"          # 状態（読み書き・無ければ初回作成）
DAILY_LOG_PATH = BASE_DIR / "ronin_daily_log.md"            # 日次ログ（追記）
...
def main():
    ...
    if os.environ.get("RONIN_REPLY_ENGINE_ENABLED") != "true":
        print("RONIN_REPLY_ENGINE_ENABLED が true でないため停止します。")
        return

    # roninは専用トークンを優先。GitHub Actionsでは THREADS_ACCESS_TOKEN にフォールバック。
    token = os.environ.get("RONIN_THREADS_ACCESS_TOKEN") or os.environ.get("THREADS_ACCESS_TOKEN")
    if not token:
        print("トークンが見つかりません（RONIN_THREADS_ACCESS_TOKEN）。停止します。")
        return
    config = load_config()
