...
from curl_cffi.requests import CurlMime # multipart/form-dataを正しく送るためのオブジェクト
...
def upload_image_to_substack(session, image_path):
    ...
    # curl_cffi は files= ではなく multipart= を使う
    with open(image_path, "rb") as f:
        multipart = {"image": (os.path.basename(image_path), f, "image/png")}
        resp = session.post(
