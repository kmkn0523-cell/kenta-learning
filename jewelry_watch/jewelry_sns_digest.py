...
SEARCH_WINDOW_DAYS = 30  # 何日前までの投稿を対象にするか（2商品限定だと少ないため30日に拡大）
...
# 対象は「サントスネックレス」と「ジュストアンクル」の2商品だけ。カルティエ全般
# （ダムール/ラブ/トリニティ等）や、同じ"サントス"でも時計は対象外にする。
# タイトルに「サントス」が入り、かつ時計ワードを含まなければサントスネックレスとみなす。
SANTOS_WATCH_WORDS = ["時計", "ウォッチ", "クロノ", "腕時計", "watch", "デュモン", "ドゥモン"]  # サントスの時計（除外用）
JUSTE_WORDS = ["ジュスト", "juste"]  # ジュストアンクル系（表記ゆれ・略称を含む）
