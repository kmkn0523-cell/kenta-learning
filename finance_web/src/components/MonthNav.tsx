...
export default function MonthNav({ selectedYear, selectedMonth, onChange }: MonthNavProps) {
  ...
  return (
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
      <button type="button" onClick={prev} style={{...STYLE_BUTTON_OUTLINE,width:44,height:44,padding:0,borderRadius:"50%",fontSize:20,display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
      <span style={{flex:1,textAlign:"center",fontSize:15,fontWeight:600}}>{selectedYear}年 {MONTH_LABELS[selectedMonth]}</span>
      <button type="button" onClick={next} style={{...STYLE_BUTTON_OUTLINE,width:44,height:44,padding:0,borderRadius:"50%",fontSize:20,display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
