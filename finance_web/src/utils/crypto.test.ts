...
describe("wrapRecoveryKey / unwrapRecoveryKey（パスワードでローカル保護）", () => {
  ...
  it("新規ラップは31万回(v2)で保存される（反復数を文字列に埋め込み）", async () => {
    const wrapped = await wrapRecoveryKey(generateRecoveryKey().raw, "pw");
    // 形式: rkw:v1:<反復数>:salt:iv:ct
    expect(wrapped.split(":")[2]).toBe(String(PBKDF2_ENC_ITERATIONS_V2));
  });
});

describe("deriveEncryptionKey（PBKDF2反復回数で別の鍵になる・段階移行の土台）", () => {
  // テスト用の固定salt（16バイト）。実コードでは getOrCreateEncSalt が生成する
  const salt = new Uint8Array(16).fill(7) as Uint8Array<ArrayBuffer>;

  it("同じパスワード・同じ反復回数なら暗号化→復号が往復一致", async () => {
    const keyA = await deriveEncryptionKey("myPassword", salt, 100_000);
    const encrypted = await encryptValue(keyA, { tx: [{ id: "1", amount: 1200 }] });
    const keyB = await deriveEncryptionKey("myPassword", salt, 100_000);
    expect(await decryptValue(keyB, encrypted)).toEqual({ tx: [{ id: "1", amount: 1200 }] });
  });

  it("反復回数が違うと鍵が変わり復号できない（旧10万回↔新31万回）", async () => {
    const legacyKey = await deriveEncryptionKey("myPassword", salt, 100_000);
    const v2Key = await deriveEncryptionKey("myPassword", salt, 310_000);
    const encryptedByLegacy = await encryptValue(legacyKey, { secret: 42 });
    // 新鍵では旧データを復号できない（だから usePersist は旧鍵フォールバックで読む）
    await expect(decryptValue(v2Key, encryptedByLegacy)).rejects.toThrow();
    // 旧鍵では読める
    expect(await decryptValue(legacyKey, encryptedByLegacy)).toEqual({ secret: 42 });
  });

  it("反復回数を省略すると v2(31万回) になる", async () => {
    const defaultKey = await deriveEncryptionKey("myPassword", salt);
    const v2Key = await deriveEncryptionKey("myPassword", salt, PBKDF2_ENC_ITERATIONS_V2);
    const encrypted = await encryptValue(v2Key, { x: "y" });
    expect(await decryptValue(defaultKey, encrypted)).toEqual({ x: "y" });
  });
});
