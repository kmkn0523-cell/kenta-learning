...
# skin リプ巡回ツール：他アカウントへのリプを「探す → 書く → 記録」まで1コマンドで回す
...
# テンプレ集は共通モジュールから読み込む。
# 直接実行（python3 skin/skin_reply_hunter.py）でも、pytest 経由でも動くように2通り試す
try:
    from skin_comment_templates import COMMENT_TEMPLATES, GENERIC_TEMPLATES  # skin/ が import パスにある時
except ImportError:
    from skin.skin_comment_templates import COMMENT_TEMPLATES, GENERIC_TEMPLATES  # リポジトリ直下から動かす時

# -----------------------------------------------
# ファイルの場所（このスクリプトと同じ skin/ フォルダにある前提）
# -----------------------------------------------
BASE_DIR = Path(__file__).resolve().parent  # このファイルがある skin/ フォルダ
TARGETS_PATH = BASE_DIR / "skin_account_targets.json"  # 巡回先10アカウントの一覧（読むだけ）
HISTORY_PATH = BASE_DIR / "skin_reply_history.json"  # アカウント別リプ履歴（読み書き・無ければ初回）
DAILY_LOG_PATH = BASE_DIR / "skin_daily_log.md"  # 日次ログ（その日のリプ件数を追記）
...
# 投稿文に含まれるキーワード → どのカテゴリーのテンプレを出すか の対応表
# （skin_comment_templates.py の6カテゴリ "1"〜"6" に合わせる）
KEYWORD_MAP = {
    "1": ["腸", "便秘", "発酵", "食物繊維", "腸活", "糖質", "食生活", "腸内"],   # 腸活・食事系
    "2": ["寝", "睡眠", "夜更かし", "寝不足", "睡眠不足", "生活リズム", "朝活"],  # 睡眠・生活習慣系
    "3": ["化粧水", "保湿", "乾燥", "スキンケア", "毛穴", "皮脂", "テカリ", "肌断食"],  # スキンケア・化粧品系
    "4": ["ニキビ", "肌荒れ", "吹き出物", "赤み", "ニキビ跡", "肌トラブル", "ゆらぎ肌"],  # ニキビ悩み・共感系
    "5": ["サプリ", "ビタミン", "亜鉛", "鉄", "栄養", "ミネラル", "プロテイン"],   # サプリ・栄養系
    "6": ["ファスティング", "断食", "16時間", "プチ断食", "オートファジー"],       # ファスティング・断食系
}
...
def update_daily_log_text(existing_text, count, today_date):
    ...
    """
    iso = today_date.isoformat()  # 例: "2026-06-02"
    weekday = WEEKDAY_JP[today_date.weekday()]  # 例: "火"
    heading = f"## {iso}（{weekday}）"  # 例: "## 2026-06-02（火）"
...
def load_targets():
    """skin_account_targets.json を読んで targets リストを返す。無い/壊れていたら None。"""
    try:
        with open(TARGETS_PATH, encoding="utf-8") as f:
            data = json.load(f)
        return data["targets"]  # 10アカウントのリスト
...
def save_history(history):
    """履歴を skin_reply_history.json に書き込む（version と更新日も一緒に保存）。"""
    payload = {
...
def append_daily_log(count, today):
    """skin_daily_log.md にその日のリプ件数を書き込む（中身の計算は update_daily_log_text）。
...
def main():
    ...
    print("\n" + "=" * 60)
    print("  skin リプ巡回ツール（探す → 書く → 記録 を1コマンドで）")
    print("=" * 60)
    print("使い方:")
    print("  1. 巡回リストから1人選ぶ → Threadsが開く")
    print("  2. 良い投稿を見つけたら本文を貼り付ける → 下書き3案が出る")
    ...
    print("（手動投稿数・気づきは夜に skin_daily_log.md へ手書きで追記してください）")
