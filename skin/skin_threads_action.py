...
import skin_note_matcher       # 投稿テーマに合う無料noteを選ぶ（CTAのnote誘導をテーマ連動に）
...
def pick_cta_note_url(count, theme="", text=""):
    """CTAローテのnote誘導回に使う記事URLを選ぶ。読めなければ空文字を返す。
    theme/text を渡すと、その投稿テーマに内容が近いnoteを優先する（テーマ連動）。
    合うものが無ければ従来どおり count で順番に選ぶ（後方互換）。"""
    try:
        with open(NOTE_PROMO_FILE, "r", encoding="utf-8") as f:
            note_data = json.load(f)
    ...
    if not articles:
        return ""
    # まず投稿テーマに内容が近いnoteを探す（読んだ話の続きを出すと押されやすい）
    matched = skin_note_matcher.match_note_url(theme, text, articles)
    if matched:
        return matched
    return articles[count % len(articles)].get("url", "")  # 合わなければ従来のローテ
...
def main(dry_run=False):
    ...
    # テーマ連動のため、テーマ名＋本文（human_post か posts全体）をマッチャーに渡す
    theme_text = thread_set.get("human_post") or " ".join(thread_set.get("posts", []))
    note_url = pick_cta_note_url(history_count, thread_set.get("theme", ""), theme_text)
