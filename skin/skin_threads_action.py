...
NOTE_PROMO_INTERVAL = 4
# 公開済みnote記事の一覧ファイル（このファイルからランダムに選ぶ）
NOTE_PROGRESS_FILE = "note_queue_progress.json"
# noteのユーザー名（公開URLの組み立てに使う）
NOTE_USERNAME = "neima"
...
NOTE_PROMO_TEMPLATES = [
    ...
    (
        "保存しておくと役立つやつ。\n\n"
        "『{title}』\n\n"
        "Threadsでは伝えきれない部分を、noteに書いてます。\n"
        "無料です👇\n\n"
        "{url}\n\n"
        "#ニキビ改善 #美肌習慣 #スキンケア #note"
    ),
    (
        "最近の投稿を読んでくれてる人に、もう1つ。\n\n"
        "『{title}』\n\n"
        "根本からケアしたい人向けに、noteで解説してます。\n"
        "完全無料👇\n\n"
        "{url}\n\n"
        "#肌荒れ #ニキビ改善 #腸活 #美肌 #note"
    ),
    (
        "「なんで肌荒れが繰り返すんだろう？」\n\n"
        "その答えの1つが、ここにあります。\n\n"
...
def should_post_note_promo(progress):
    """今回の投稿をnote宣伝に差し替えるか判定する（4回に1回）"""
    daily_index = progress.get("daily_index", 0)
    return daily_index % NOTE_PROMO_INTERVAL == 0


def build_public_note_url(post_url):
    """editor.note.com形式のURLを公開URL（note.com/neima/n/...）に変換する"""
    match = re.search(r"/(?:notes|n)/([A-Za-z0-9]+)", post_url)
    if not match:
        return post_url  # 想定外の形式ならそのまま返す
...
def pick_random_note(progress):
    ...
    try:
        with open(NOTE_PROGRESS_FILE, "r", encoding="utf-8") as f:
    ...
    candidates = [h for h in history if h.get("article") != last_promo]
    if not candidates:
        candidates = history  # 1記事しかない場合はそのまま使う
...
def post_note_promo(progress, now):
    ...
    promo_text = build_note_promo_text(title, public_url)
    ...
    # 進捗を更新する（投稿成功後のみ → 失敗時はインデックスが進まない）
    progress["daily_index"] += 1
    progress["last_note_promo_article"] = note.get("article", "")
...
def main():
    ...
    # note宣伝投稿の判定（4回に1回、note宣伝に差し替える）
    if should_post_note_promo(progress):
        if post_note_promo(progress, now):
            return  # 宣伝投稿が成功したら終了
        # 失敗した場合は通常投稿にフォールバックする
