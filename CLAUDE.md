...
## 表示・UIの修正（ステータスライン等）※今回の反省から
- **着手前に実画面を1回確認する**。推測で直し始めない。具体的には次のどちらかを先にやる:
  - ユーザーに `!` でコマンドを実行してもらい実出力を見る（例: `! echo '<JSON>' | bash ~/.claude/statusline-command.sh`）
  - 実画面のスクショをもらう（WSLなら `powershell.exe` でクリップボード画像を保存→Readで読む手もある）
- **現状を言葉で先に確認する**。「今、画面に何行見えて、各行に何がある?」を1問だけ聞く。これだけで構造の誤解（下記）を防げる
- **ステータスラインは2層構造**: 上の行=自前の `~/.claude/statusline-command.sh` の出力 / 下の行=Claude Code組み込みフッター（モデル・コンテキスト等、消せない）。自前スクリプトに出すのは使用量(5h/7d)など組み込みに無い情報だけでよい
- **Bashの標準出力は信用しすぎない**。出力が乱れたら、ファイルに書き出して `Read` ツールで読むか、ユーザーの `!` 実行に切り替える。Edit/Writeの反映は信頼できるが、実行結果(stdout)が壊れることがある
- 同じ修正で2回以上空振りしたら、推測をやめて必ず実機確認に切り替える
